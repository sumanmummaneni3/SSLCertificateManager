...
**Update (2026-07-25):** Phase 1+2 shipped (see R26). Wrong-org-at-login is fixed and an explicit switcher exists; only the Phase-3 targeted backfill remains before N16 is fully closed.
...
| R26 (new) | Multi-org users pinned to home org at login; wrong org shown, invited-org targets hidden; no org-switcher | ~~HIGH~~ | **Mostly closed (RFC 0015 Phase 1+2 shipped, 2026-07-25, main @ 91dec06)** — Phase 1 (PR #25): `users.last_active_org_id` (V45) + shared `ActiveOrgResolver`, stamped on invite-accept and read at login in **both** issuance paths (server HS256 `OAuth2AuthenticationSuccessHandler`, auth-service RS256 `AuthProvisioningService`) — closes the wrong-org bug and the ADMIN-default privilege leak. Phase 2 (PR #27): explicit switch — authoritative `POST /api/auth/switch-org` (auth-service, RS256; re-mints + deletes the presenting session), local/dev-only `POST /api/v1/auth/switch-org` (server, HS256, not gateway-valid per RFC §4), and a Sidebar org-switcher driven by `/api/v1/auth/me` memberships (now filtered to ACCEPTED + non-revoked). **Residual:** Phase 3 targeted backfill of pre-Phase-1 stuck users (they already self-heal on next login via resolver step 3; backfill makes it proactive) — task #4. RFC §7 "which JWT path is live in prod" is a deploy-config note, not a code gap (both paths fixed identically) |
