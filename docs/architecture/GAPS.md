...
**Update (2026-07-25) — CLOSED:** All three RFC 0015 phases shipped (Phase 1 login resolver + `last_active_org_id`; Phase 2 switch-org endpoints + UI switcher; Phase 3 targeted backfill). Wrong-org-at-login is fixed, an explicit switcher exists, and pre-Phase-1 stuck users are rescued. N16 resolved.
...
| R26 (new) | Multi-org users pinned to home org at login; wrong org shown, invited-org targets hidden; no org-switcher | ~~HIGH~~ | **Closed (RFC 0015 Phases 1–3, 2026-07-25)** — Phase 1 (PR #25): `users.last_active_org_id` (V45) + shared `ActiveOrgResolver`, stamped on invite-accept, read at login in **both** issuance paths (server HS256 `OAuth2AuthenticationSuccessHandler`, auth-service RS256 `AuthProvisioningService`); also closed the ADMIN-default privilege leak. Phase 2 (merged @91dec06): authoritative `POST /api/auth/switch-org` (auth-service RS256; re-mints + deletes the presenting session), local/dev-only server `POST /api/v1/auth/switch-org` (HS256, not gateway-valid, RFC §4), Sidebar org-switcher driven by `/api/v1/auth/me` memberships (filtered to ACCEPTED + non-revoked). Phase 3 (PR #29): idempotent, dry-run-able, boot-safe targeted backfill (`BackfillLastActiveOrgService` + `LastActiveOrgBackfillRunner`, `APP_BACKFILL_LAST_ACTIVE_ORG_MODE=off\|dry-run\|apply`) setting `last_active_org_id` to the most-recent ACCEPTED non-revoked membership for pre-Phase-1 stuck users only (home membership missing/revoked + a valid membership elsewhere); never touches valid-home-membership users or platform admins. RFC §7 "which JWT path is live in prod" remains an informational deploy-config note, not a reopener |
