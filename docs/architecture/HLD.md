...
9. **Role-aware JWT TTL, no refresh tokens, only per-org revocation.** Tokens are RS256 from the auth-service: normal users get a 24h absolute TTL (`auth.jwt.expiration-ms` / `AUTH_JWT_EXPIRATION_MS`), platform admins get a non-expiring token (far-future exp ~100 years) per product decision. There is no refresh-token mechanism. Per-org revocation exists via the `revoked_tokens` table (migration `V25`), checked in `JwtAuthenticationFilter`, but there is no per-session kill switch on the gateway hot path — so a leaked token (especially a non-expiring admin token) cannot be individually revoked mid-life. Idle/expiry handling in the UI (30-min idle timeout, proactive on-navigation `exp` check) is client-side UX only; the gateway's `exp` enforcement is the real bound. Agent key revocation works via the status check in `AgentAuthFilter`.
