...
/**
 * Authenticates requests from certguard-renewal-service on /internal/v1/** paths.
 * Validates "Authorization: Bearer <token>" against INTERNAL_SERVICE_TOKEN env var.
 */
@Slf4j
@Component
public class InternalServiceAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(@NonNull HttpServletRequest request,
        ...
        if (serviceToken.isBlank() || !serviceToken.equals(token)) {
            log.warn("Internal service auth failed for URI: {}", request.getRequestURI());
